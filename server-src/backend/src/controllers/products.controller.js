...
const list = asyncHandler(async (req, res) => {
  ...
  const totalStockExpr = `COALESCE((SELECT SUM(s.current_qty) FROM stock s WHERE s.product_id = p.id AND s.store_id = ${Number(req.user.store_id)||1}), 0)`;
...
const getOne = asyncHandler(async (req, res) => {
  const { rows } = await query(
    `SELECT p.*, c.name AS category_name, b.name AS brand_name, u.short_code AS unit_code,
            COALESCE((SELECT SUM(s.current_qty) FROM stock s WHERE s.product_id = p.id AND s.store_id = ${Number(req.user.store_id)||1}), 0) AS total_stock
     FROM products p
     LEFT JOIN categories c ON c.id = p.category_id
     LEFT JOIN brands b ON b.id = p.brand_id
     LEFT JOIN units u ON u.id = p.unit_id
...
const create = asyncHandler(async (req, res) => {
  ...
  const hsnCode = b.hsnCode == null ? '' : String(b.hsnCode).trim();
  if (hsnCode.length > 10) throw ApiError.badRequest('HSN code must be 10 characters or fewer');
  if (hsnCode && !/^[0-9A-Za-z-]+$/.test(hsnCode)) throw ApiError.badRequest('HSN code may contain only letters, numbers, and hyphens');

  const gstRate = b.gstRate == null || b.gstRate === '' ? 0 : Number(b.gstRate);
  ...
  const result = await withTransaction(async (client) => {
    let brandId = b.brandId || null;
    const brandName = typeof b.brandName === 'string' ? b.brandName.trim() : '';
    if (brandName) {
      if (brandName.length > 100) throw ApiError.badRequest('Brand name must be 100 characters or fewer');
      const existingBrand = await client.query('SELECT id FROM brands WHERE LOWER(name) = LOWER($1) LIMIT 1', [brandName]);
      if (existingBrand.rows[0]) brandId = existingBrand.rows[0].id;
      else brandId = (await client.query('INSERT INTO brands (name) VALUES ($1) RETURNING id', [brandName])).rows[0].id;
    }
    const { rows } = await client.query(
      ...
      [
        ...
        brandId, b.supplierId || null, b.unitId, hsnCode || null, gstRate,
    ...
    const auto=(await client.query("SELECT value FROM settings WHERE key='online_store_auto_list'")).rows[0]?.value===true;
    if(auto){await client.query('UPDATE products SET is_online_visible=true WHERE id=$1',[product.id]);product.is_online_visible=true;}
...
const update = asyncHandler(async (req, res) => {
  ...
  if (b.gstRate !== undefined && (!Number.isFinite(Number(b.gstRate)) || Number(b.gstRate) < 0 || Number(b.gstRate) > 100)) {
    throw ApiError.badRequest('GST rate must be between 0 and 100');
  }

  let brandId = b.brandId;
  const brandName = typeof b.brandName === 'string' ? b.brandName.trim() : '';
  if (brandName) {
    if (brandName.length > 100) throw ApiError.badRequest('Brand name must be 100 characters or fewer');
    const existingBrand = await query('SELECT id FROM brands WHERE LOWER(name) = LOWER($1) LIMIT 1', [brandName]);
    brandId = existingBrand.rows[0]?.id;
    if (!brandId) brandId = (await query('INSERT INTO brands (name) VALUES ($1) RETURNING id', [brandName])).rows[0].id;
  }
