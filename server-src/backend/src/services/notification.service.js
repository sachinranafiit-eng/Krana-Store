...
async function sendTemplatedMessage({ channel, to, templateCode, variables = {}, partyType, partyId, userId }) {
  ...
  if (channel === 'both') {
    const results = await Promise.all(['whatsapp', 'sms'].map((item) => sendTemplatedMessage({
      channel: item, to, templateCode, variables, partyType, partyId, userId,
    })));
    return { status: results.every((r) => r.status === 'sent') ? 'sent' : 'partial', results };
  }
