...
async function init(){
 ...
 if(memory) await exec(`CREATE TABLE IF NOT EXISTS auth_sessions(id TEXT PRIMARY KEY,user_id INT REFERENCES users(id),refresh_hash TEXT,expires_at TIMESTAMPTZ NOT NULL,created_at TIMESTAMPTZ DEFAULT now()); CREATE TABLE IF NOT EXISTS held_carts(id SERIAL PRIMARY KEY,store_id INT REFERENCES stores(id),name TEXT NOT NULL,cart JSONB NOT NULL,created_by INT REFERENCES users(id),created_at TIMESTAMPTZ DEFAULT now()); CREATE TABLE IF NOT EXISTS sale_tenders(id SERIAL PRIMARY KEY,sale_id INT REFERENCES sales(id),mode TEXT NOT NULL,amount NUMERIC(14,2) NOT NULL); CREATE SEQUENCE IF NOT EXISTS invoice_seq; CREATE SEQUENCE IF NOT EXISTS purchase_seq; CREATE SEQUENCE IF NOT EXISTS po_seq; ALTER TABLE sales ADD COLUMN IF NOT EXISTS request_key TEXT; ALTER TABLE sales ADD COLUMN IF NOT EXISTS notes TEXT; ALTER TABLE purchases ADD COLUMN IF NOT EXISTS request_key TEXT; ALTER TABLE sales_returns ADD COLUMN IF NOT EXISTS sale_item_id INT; ALTER TABLE sales_returns ADD COLUMN IF NOT EXISTS taxable_amount NUMERIC(14,2) DEFAULT 0; ALTER TABLE sales_returns ADD COLUMN IF NOT EXISTS cost_amount NUMERIC(14,2) DEFAULT 0; ALTER TABLE sales_returns ADD COLUMN IF NOT EXISTS refund_mode TEXT DEFAULT 'credit'; ALTER TABLE purchase_returns ADD COLUMN IF NOT EXISTS purchase_item_id INT; ALTER TABLE purchase_returns ADD COLUMN IF NOT EXISTS taxable_amount NUMERIC(14,2) DEFAULT 0; ALTER TABLE payments ADD COLUMN IF NOT EXISTS store_id INT; ALTER TABLE payments ADD COLUMN IF NOT EXISTS notes TEXT; ALTER TABLE purchase_orders ADD COLUMN IF NOT EXISTS items JSONB DEFAULT '[]'; ALTER TABLE purchase_orders ADD COLUMN IF NOT EXISTS expected_date DATE; ALTER TABLE purchase_orders ADD COLUMN IF NOT EXISTS notes TEXT; INSERT INTO settings(key,value) VALUES ('online_store_enabled','true'),('online_store_auto_list','true') ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value;`);
 ...
 const migrations=memory?['004_bulk.sql','005_customer_accounts.sql','006_online_payments_alerts.sql']:['003_operations.sql','004_bulk.sql','005_customer_accounts.sql','006_online_payments_alerts.sql','007_online_store_repair.sql'];
